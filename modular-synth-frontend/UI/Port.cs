...
public class Port : Component
{
	...
	public static List<Port> ports = new();
	...
	public Dictionary<Port, Wire> portsConnectedTo = new();

	public Vector2 Position { get => worldSpacePosition; private set { worldSpacePosition = value; } }
		...
		if (canInteract)
		{
			if (screenSpaceBoundingBox.Contains(input.MousePosition()))
			{
				...
				if (input.LeftMouseClickDown())
				{
					...
					dragging = true;
					clickOffset = worldSpacePosition - input.MousePosVector();
				...
				else if (input.LeftMouseClickUp() && !dragging)
				{
					...
					if (portToConnectFrom != null)
					{
						...
						if (portIsConnected(portToConnectFrom))
						{ //delete connections to same port
